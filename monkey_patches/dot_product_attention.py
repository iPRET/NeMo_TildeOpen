# Ingus completely just overrode DotProductAttention.forward to always use flash_attn_varlen_func.
...
class DotProductAttention(TransformerEngineBaseModule):
    ...
    def forward(
        ...
        #Complete yolo overriding all the attentio logic.
        #This definitely breaks FP8 and context parallelism, but maybe other things too.
        #I'm not even sure.
        sequence_length = query_layer.shape[0]
        batch_size = query_layer.shape[1]
        q_head_count = query_layer.shape[2]
        head_size = query_layer.shape[3]
        kv_head_count = key_layer.shape[2]

        query_layer = query_layer.permute(1, 0, 2, 3) #[B, S, Heads, Hidden]
        key_layer = key_layer.permute(1, 0, 2, 3) #[B, S, HeadsGQA, Hidden]
        value_layer = value_layer.permute(1, 0, 2, 3)

        query_layer = query_layer.reshape([-1, q_head_count, head_size])
        key_layer = key_layer.reshape([-1, kv_head_count, head_size])
        value_layer = value_layer.reshape([-1, kv_head_count, head_size])
        ...
        #Flash go brrrrrrrrrrrrrrrrrrrr
        ...
        output = output.reshape([batch_size, sequence_length, head_size * q_head_count]).permute(1, 0, 2)
        ...
        with torch.cuda.device(query_layer.device), self.prepare_forward(
            ...
            # run attention
            #print("About to do TensorEngine flash attention if check")
            use_flash_attention = True
            if use_flash_attention:
                #print("Got inside the TensorEngine use_flash_attention branch")
                if core_attention_bias_type == "alibi":
            ...
            if use_fused_attention:
                #print("Got inside the TensorEngine fused attention branch")
            ...
            if use_unfused_attention:
                print("Got inside the tensorengine unfused attention branch")
                if checkpoint_core_attention:
                    return self._checkpointed_attention_forward(
